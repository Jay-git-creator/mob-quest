...
/** Map Width and Height (variables) **/
let MAP_WIDTH;
let MAP_HEIGHT;
...
function init() {
  ...
  // Calculate MAP_WIDTH and MAP_HEIGHT after canvas dimensions are set
  MAP_WIDTH = Math.ceil(canvas.width / TILE_SIZE);
  MAP_HEIGHT = Math.ceil(canvas.height / TILE_SIZE);
...
window.addEventListener('resize', function() {
  ...
  // Recalculate MAP_WIDTH and MAP_HEIGHT
  MAP_WIDTH = Math.ceil(canvas.width / TILE_SIZE);
  MAP_HEIGHT = Math.ceil(canvas.height / TILE_SIZE);

  // Reposition the office and player
  office = {
    x: Math.floor(MAP_WIDTH / 2) * TILE_SIZE,
    y: Math.floor(MAP_HEIGHT / 2) * TILE_SIZE
  };
  ...
  // Regenerate map and reposition elements
  generateMap();
  placeRestaurants();
});
